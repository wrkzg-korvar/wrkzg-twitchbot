import { useState } from "react";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { Pencil, Plus, Trash2 } from "lucide-react";
import { timersApi } from "../api/timers";
import { PageHeader } from "../components/ui/PageHeader";
import { SmartDataTable } from "../components/ui/DataTable";
import type { SmartColumn } from "../components/ui/DataTable";
import { ConfirmDialog } from "../components/ui/ConfirmDialog";
import { TimerForm } from "../components/features/timers/TimerForm";
import { showToast } from "../hooks/useToast";
import { useModuleLock } from "../hooks/useModuleLock";
import { LockBanner } from "../components/ui/LockBanner";
import type { TimedMessage } from "../types/timers";

export function TimersPage() {
  const queryClient = useQueryClient();
  const { isLocked, lockReason } = useModuleLock("/timers");
  const [showCreate, setShowCreate] = useState(false);
  const [editingTimer, setEditingTimer] = useState<TimedMessage | null>(null);
  const [deleteId, setDeleteId] = useState<number | null>(null);

  const { data: timers, isLoading } = useQuery<TimedMessage[]>({
    queryKey: ["timers"],
    queryFn: timersApi.getAll,
  });

  const toggleMutation = useMutation({
    mutationFn: (timer: TimedMessage) =>
      timersApi.update(timer.id, { isEnabled: !timer.isEnabled }),
    onSuccess: (_data, timer) => {
      showToast("success", `Timer "${timer.name}" ${timer.isEnabled ? "disabled" : "enabled"}`);
      queryClient.invalidateQueries({ queryKey: ["timers"] });
    },
    onError: (err: Error) => showToast("error", err.message),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => timersApi.remove(id),
    onSuccess: () => {
      showToast("success", "Timer deleted");
      queryClient.invalidateQueries({ queryKey: ["timers"] });
      setDeleteId(null);
    },
    onError: (err: Error) => showToast("error", err.message),
  });

  const columns: SmartColumn<TimedMessage>[] = [
    {
      key: "name",
      header: "Name",
      sortable: true,
      searchable: true,
    },
    {
      key: "messages",
      header: "Messages",
      searchable: true,
      render: (_, row) => (
        <span>
          {row.messages[0]}
          {row.messages.length > 1 && (
            <span className="text-[var(--color-text-muted)] text-xs ml-1">
              +{row.messages.length - 1} more
            </span>
          )}
        </span>
      ),
    },
    {
      key: "intervalMinutes",
      header: "Interval",
      sortable: true,
      className: "text-right",
      render: (v) => <>{v} min</>,
    },
    {
      key: "minChatLines",
      header: "Min Lines",
      sortable: true,
      className: "text-right",
    },
    {
      key: "isEnabled",
      header: "Enabled",
      sortable: true,
      render: (_, row) => (
        <button
          type="button"
          role="switch"
          aria-checked={row.isEnabled}
          disabled={isLocked}
          onClick={(e) => {
            e.stopPropagation();
            toggleMutation.mutate(row);
          }}
          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors disabled:opacity-40 ${
            row.isEnabled ? "bg-[var(--color-brand)]" : "bg-[var(--color-elevated)]"
          }`}
        >
          <span
            className={`inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform ${
              row.isEnabled ? "translate-x-4" : "translate-x-0.5"
            }`}
          />
        </button>
      ),
    },
    {
      key: "runWhenOnline",
      header: "When",
      render: (_, row) => (
        <div className="flex gap-1">
          {row.runWhenOnline && (
            <span className="inline-block rounded px-1.5 py-0.5 text-xs bg-green-500/10 text-green-400">
              Online
            </span>
          )}
          {row.runWhenOffline && (
            <span className="inline-block rounded px-1.5 py-0.5 text-xs bg-blue-500/10 text-blue-400">
              Offline
            </span>
          )}
        </div>
      ),
    },
    {
      key: "actions",
      header: "Actions",
      render: (_, row) => (
        <div className="flex items-center gap-1.5">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setShowCreate(false);
              setEditingTimer(row);
            }}
            disabled={isLocked}
            className="rounded p-1 text-[var(--color-text-muted)] hover:bg-[var(--color-elevated)] hover:text-[var(--color-text)] transition-colors disabled:opacity-40"
            title="Edit"
          >
            <Pencil className="h-3.5 w-3.5" />
          </button>
          <button
            onClick={(e) => {
              e.stopPropagation();
              setDeleteId(row.id);
            }}
            disabled={isLocked}
            className="rounded p-1 text-[var(--color-text-muted)] hover:bg-[var(--color-elevated)] hover:text-red-400 transition-colors disabled:opacity-40"
            title="Delete"
          >
            <Trash2 className="h-3.5 w-3.5" />
          </button>
        </div>
      ),
    },
  ];

  return (
    <div className="space-y-6 p-6">
      {lockReason && <LockBanner message={lockReason} />}
      <PageHeader
        title="Timers"
        description="Schedule recurring messages to keep your chat active."
        helpKey="timed-messages"
        actions={
          !showCreate && !editingTimer ? (
            <button
              onClick={() => setShowCreate(true)}
              disabled={isLocked}
              className="flex items-center gap-1.5 rounded-lg bg-[var(--color-brand)] px-4 py-2 text-sm font-medium text-[var(--color-bg)] hover:bg-[var(--color-brand-hover)] transition-colors disabled:opacity-40"
            >
              <Plus className="h-4 w-4" /> New Timer
            </button>
          ) : undefined
        }
      />

      {showCreate && (
        <TimerForm onClose={() => setShowCreate(false)} />
      )}

      {editingTimer && (
        <TimerForm
          initial={editingTimer}
          onClose={() => setEditingTimer(null)}
        />
      )}

      <SmartDataTable<TimedMessage>
        data={timers ?? []}
        columns={columns}
        pageSize={50}
        searchPlaceholder="Search timers..."
        emptyMessage="No timers yet."
        isLoading={isLoading}
        getRowKey={(row) => row.id}
      />

      <ConfirmDialog
        open={deleteId !== null}
        title="Delete Timer"
        message="Are you sure you want to delete this timer? This action cannot be undone."
        confirmLabel="Delete"
        onConfirm={() => deleteId && deleteMutation.mutate(deleteId)}
        onCancel={() => setDeleteId(null)}
      />
    </div>
  );
}
