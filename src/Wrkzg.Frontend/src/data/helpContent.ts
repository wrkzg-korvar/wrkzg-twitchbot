...
export const helpContent: Record<string, HelpEntry> = {
  ...
  "effects": {
    title: "Automations",
    description: "Build custom reactions to events, chat commands, Channel Point redemptions, and more. Each automation has a trigger, optional conditions, and a chain of actions.",
    howToUse: [
      "Click 'New Automation' to open the Visual Builder",
      "Pick a trigger (when should the automation fire?)",
      "Add optional conditions (who/when is allowed to trigger it?)",
      "Add actions (what should happen?)",
      "Use variables like {user} in text — they are substituted automatically",
      "The 'Test' button runs only THIS automation (for testing)",
      "Switch to 'JSON' for the raw JSON configuration",
    ],
    templateVariables: [
      { variable: "{user}", description: "Name of the viewer" },
      { variable: "{args}", description: "Text after the command" },
      { variable: "{target}", description: "First word after the command" },
      { variable: "{viewers}", description: "Viewer count for raids" },
      { variable: "{tier}", description: "Subscription tier" },
      { variable: "{months}", description: "Subscription months for resubs" },
      { variable: "{count}", description: "Number of gifted subs" },
      { variable: "{message}", description: "Message on resubs" },
      { variable: "{reward}", description: "Name of the Channel Points reward" },
      { variable: "{input}", description: "User input for Channel Points" },
      { variable: "{cost}", description: "Cost of the Channel Points reward" },
      { variable: "{broadcaster}", description: "Streamer's name" },
      { variable: "{hotkey}", description: "Hotkey key combination" },
    ],
    handbookSection: "5.4",
  },
  ...
  "import": {
    ...
    howToUse: [
      ...
      "Imports run in the background — you can leave the page and keep working",
      "Progress is shown in the notification bell (sidebar)",
      "While an import is running, affected pages are read-only (lock banner)",
    ],
  ...
  "integrations": {
    title: "Integrations",
    description: "Connect external services to your bot. Discord for chat notifications, OBS for scene control.",
    howToUse: [
      "Discord: create a webhook in your Discord server and paste the URL here",
      "OBS: enable the WebSocket Server in OBS (Tools -> WebSocket Server Settings)",
      "OBS: enter the host (localhost), port (4455), and optional password",
      "Once connected, you can use OBS actions in Automations and Hotkeys",
    ],
    handbookSection: "5.5",
  },
  ...
  "permissions": {
    title: "Permissions",
    description: "Overview of which chat commands require which user role.",
    howToUse: [
      "Roles are created on the Roles page and assigned automatically or manually",
      "Higher roles include lower ones (a moderator is also a viewer)",
      "System commands have fixed minimum roles",
      "Custom commands can be restricted via Automations with role conditions",
    ],
    handbookSection: "4.5",
  },
